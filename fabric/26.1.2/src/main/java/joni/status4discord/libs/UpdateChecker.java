...
public class UpdateChecker {
     ...
     */
    public UpdateChecker(String projectId, String currentVersion, List<String> loaders, List<String> gameVersions) {
        this.logger = Status4discord.LOGGER;
        this.userAgent = Status4discord.MOD_ID + "/" + currentVersion;
    ...
    private CompletableFuture<VersionInfo> checkForUpdate() {
        ...
        return CompletableFuture.supplyAsync(this::fetchLatest, executor)
                .whenComplete((info, ex) -> {
                    if (ex != null) {
                        logger.warn("Failed to check for update via Modrinth: {}", ex.getMessage());
                        return;
                    }
                    ...
                    if (info != null) {
                        ...
                        if (isNewerThanCurrent(info)) {
                            logger.info("A new version is available! Current: {} Latest: {}", currentVersion, info.versionNumber());
                            logger.info("https://modrinth.com/plugin/{}/version/{}", projectId, info.versionNumber());
                        } else {
    ...
    private VersionInfo fetchLatest() {
        try {
            ...
            try (InputStreamReader reader = new InputStreamReader(conn.getInputStream())) {
                ...
                if (versions.isEmpty()) {
                    logger.warn("No versions found for Modrinth project {} matching the given loaders/game versions", projectId);
                    return null;
                }
            ...
            logger.warn("Modrinth update check failed: {}", ex.getMessage());
            return null;
        }
    }
